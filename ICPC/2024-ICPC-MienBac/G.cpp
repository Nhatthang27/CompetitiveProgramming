...
void solve()
{
    ...
    while (mp.size() > 0)
    {
        ...
        // }
        // cerr << endl;
        ans = max(ans, (int)(mp.begin()->ff[0] * (mp.size() + 1) + mp.size()));
        ...
        if (pre_index >= 1)
        {
            ...
            mp.erase({mi[pre_index], ma[pre_index], pre_index});
            ...
            mp[{mi[pre_index], ma[pre_index], pre_index}] = pre_index;
        ...
        if (next_index <= n)
        {
            mp.erase({mi[next_index], ma[next_index], next_index});
            ...
            mp[{mi[next_index], ma[next_index], next_index}] = next_index;
            l_index[next_index] = pre_index;
            if (pre_index >= 1)
                r_index[pre_index] = next_index;
        }
